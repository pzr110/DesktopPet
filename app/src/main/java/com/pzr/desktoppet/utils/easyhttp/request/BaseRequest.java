...
package com.pzr.desktoppet.utils.easyhttp.request;
...
import com.pzr.desktoppet.utils.easyhttp.api.ApiService;
import com.pzr.desktoppet.utils.easyhttp.cache.RxCache;
import com.pzr.desktoppet.utils.easyhttp.cache.converter.IDiskConverter;
import com.pzr.desktoppet.utils.easyhttp.cache.model.CacheMode;
import com.pzr.desktoppet.utils.easyhttp.https.HttpsUtils;
import com.pzr.desktoppet.utils.easyhttp.interceptor.BaseDynamicInterceptor;
import com.pzr.desktoppet.utils.easyhttp.interceptor.CacheInterceptor;
import com.pzr.desktoppet.utils.easyhttp.interceptor.CacheInterceptorOffline;
import com.pzr.desktoppet.utils.easyhttp.interceptor.HeadersInterceptor;
import com.pzr.desktoppet.utils.easyhttp.interceptor.NoCacheInterceptor;
import com.pzr.desktoppet.utils.easyhttp.model.HttpHeaders;
import com.pzr.desktoppet.utils.easyhttp.model.HttpParams;
import com.pzr.desktoppet.utils.easyhttp.utils.HttpLog;
import com.pzr.desktoppet.utils.easyhttp.utils.RxUtil;
...
import static com.pzr.desktoppet.utils.easyhttp.EasyHttp.getRetrofitBuilder;
import static com.pzr.desktoppet.utils.easyhttp.EasyHttp.getRxCache;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    protected boolean accessToken = true;                                   //是否需要追加token
    ...
    public R cacheTime(long cacheTime) {
        if (cacheTime <= -1)
            cacheTime = EasyHttp.DEFAULT_CACHE_NEVER_EXPIRE;
    ...
    public R params(String key, String value) {
        if (value == null) {
            value="";
            HttpLog.e("params(BaseRequest.java:334)--> "+String.format("参数：%s，值是空的", key));
        }
    ...
    /**
     * @param key 将所有的请求参数合成json 当作传入参数  key 的value
     * @return
     */
    public R paramsValueToJson(String key) {
        Gson gson = new Gson();
        ...
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && sslParams == null
            ...
            for (Interceptor interceptor : builder.interceptors()) {
                if (interceptor instanceof BaseDynamicInterceptor) {
                    ((BaseDynamicInterceptor) interceptor).accessToken(accessToken);
                }
            }
            return builder;
        } else {
            ...
            if (cookies.size() > 0)
                EasyHttp.getCookieJar().addCookies(cookies);
