...
package com.pzr.desktoppet.utils.easyhttp.cache.core;
...
public class CacheCore {
    ...
    public synchronized <T> T load(Type type, String key, long time) {
        ...
        HttpLog.e("loadCache  key=" + cacheKey);
        if (disk != null) {
    ...
    public synchronized <T> boolean save(String key, T value) {
        String cacheKey = ByteString.of(key.getBytes()).md5().hex();
        HttpLog.e("saveCache  key=" + cacheKey);
    ...
    public synchronized boolean containsKey(String key) {
        String cacheKey = ByteString.of(key.getBytes()).md5().hex();
        HttpLog.e("containsCache  key=" + cacheKey);
        if (disk != null) {
    ...
    public synchronized boolean remove(String key) {
        String cacheKey = ByteString.of(key.getBytes()).md5().hex();
        HttpLog.e("removeCache  key=" + cacheKey);
        if (disk != null) {
